...
let age = 22;
let discount =
	age < 18 ? 10 : age > 65 ? 30 : age >= 18 ? 20 : "Некорректный возраст";
...
const typeStandard = "Стандарт";
const typeExpress = "Экспресс";
const typePremium = "Премиум";
...
console.log(`Вес посылки: ${weight}`);

if (
	deliveryType != typeStandard &&
	deliveryType != typeExpress &&
	deliveryType != typePremium
) {
...
const baseDeliveryPrice =
	weight < 1 ? 5 : weight > 5 ? 15 : weight >= 1 ? 10 : "Произошла ошибка";

console.log(`Базовая стоимость доставки: ${baseDeliveryPrice}$`);

let priceCoefficient;
switch (true) {
	case deliveryType == typeStandard:
		priceCoefficient = 1;
		break;
	case deliveryType == typeExpress:
		priceCoefficient = 1.5;
		break;
	case deliveryType == typePremium:
		priceCoefficient = 2;
		break;
	default:
		priceCoefficient = "Ошибка рассчёта";
}

console.log(`Коэффицент доставки: ${priceCoefficient}`);

let totalDeliveryPrice = baseDeliveryPrice * priceCoefficient;

alert(`Итоговая стоимость доставки: ${totalDeliveryPrice}$.`);
